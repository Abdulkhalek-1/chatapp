...
export default function RootLayout() {
  ...
  const { setUserData, authenticated, loading, ...userData } = useUserData(
    (store) => store,
  );
  ...
  useEffect(() => {
    ...
    try {
      baseAPI.get("/users/profile/").then((res) =>
        setUserData({
          ...res.data,
          authenticated: true,
        }),
  ...
  // if (!loading) if (!authenticated) return <Navigate to="/accounts/login" />;
